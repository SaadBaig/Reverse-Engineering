...
We can see functions in here from our function list, namely ```InternetOpenA```, ```InternetOpenUrlA``` and ```InternetCloseHandle```. We can see that there is an if function that does a comparison between ```iVar2``` value which you can see being assigned to ```InternetOpenUrlA```. If this comparison == 0 then it eventually calls another function ```FUN_00408090```. Considering the outside for loop does nothing, this function seems to be the entry if our kill switch doesn’t ring true; the real entry. Lets rename it ```wannacry_entry``` and take a look:
